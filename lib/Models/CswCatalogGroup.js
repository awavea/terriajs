        ...
        if (that.includeWps) {
          ...
          for (var k = 0; k < uris.length; ++k) {
            ...
            if (wpsuri.protocol.indexOf("OGC:WPS") > -1) {
              ...
              var wpsPromise = createWpsItemFromUri(
                that.terria,
                wpsuri.toString(),
                record.title
              );
          ...
          for (var m = 0; m < uris.length; m++) {
            ...
            if (!excludedProtocol) {
              ...
              if (
                url.description === "GetFeatureInfoUrl" &&
                url.protocol.indexOf("OGC:WPS") === 0
              ) {
                getFeatureInfoUrl = url.toString();
              }
              downloadUrls.push({
                url: url.toString(),
                description: defined(url.description)
                  ? url.description
                  : url.name
              });
            }
  ...
  if (defined(catalogItem)) {
    if (catalogItem instanceof WebProcessingServiceCatalogGroup) {
      ...
      if (defined(getFeatureInfoUrl)) {
        createPickFeaturesUsingWebProcessingService(
          catalogGroup.terria,
          catalogItem,
          getFeatureInfoUrl
        );
